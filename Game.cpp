#include "Game.h"
void Game::GetCountryChess(uint32_t srcCountry, uint32_t dstCountry){
#ifndef HAN_CAN_PLAY
...
void Game::InitWuChessRowAndColumn(Chess *pChess[DRAW_COUNTRY_CHESS_COUNT]){
...
void Game::InitShuChessRowAndColumn(Chess *pChess[DRAW_COUNTRY_CHESS_COUNT]){
    ...
    pChess[SHI_CHESS_INDEX]->SetPos(pChess[JIANG_CHESS_INDEX]->GetRow(), pChess[MA_CHESS_INDEX]->GetColumn() + 2);
...
void Game::InitWeiChessRowAndColumn(Chess *pChess[DRAW_COUNTRY_CHESS_COUNT]){
...
void Game::InitHanChessRowAndColumn(Chess *pChess[DRAW_COUNTRY_CHESS_COUNT]){
...
void Game::InitHanChessRowAndColumn(uint32_t country, Chess *pChess[DRAW_COUNTRY_CHESS_COUNT]){
...
void Game::InitHanChessinfo(Chess *pChess[DRAW_COUNTRY_CHESS_COUNT]){
...
void Game::InitChessInfo(uint32_t country, Chess *pChess[DRAW_COUNTRY_CHESS_COUNT]){
...
#ifndef HAN_CAN_PLAY
    InitHanChessRowAndColumn(playerCountry, mChess[HAN_COUNTRY_INDEX]);
#endif
...
#ifdef HAN_CAN_PLAY
        InitWeiChessRowAndColumn(mChess[HAN_COUNTRY_INDEX]);
#endif
    }
...
#ifdef HAN_CAN_PLAY
        InitWuChessRowAndColumn(mChess[HAN_COUNTRY_INDEX]);
#endif
    }
...
#ifdef HAN_CAN_PLAY
        InitHanChessRowAndColumn(mChess[HAN_COUNTRY_INDEX]);
#endif
    }
#ifdef HAN_CAN_PLAY
...
Chess *Game::GetChess(uint32_t row, uint32_t column)const{
...
Chess *Game::GetChess(uint32_t country, const glm::vec2&mousePos)const{
    Chess *pChess = nullptr;
...
Chess *Game::GetChess(uint32_t country, uint32_t row, uint32_t column)const{
    Chess *pChess = nullptr;
...
void Game::DestroyCountry(uint32_t country){
...
void Game::CaptureChess(const Chess *pChess, const Chess *pTarget){
    if(pTarget){
        const uint32_t dstCountry = pTarget->GetCountry(), dstChess = pTarget->GetChess();
#ifndef HAN_CAN_PLAY
        if(pTarget->GetChess() == JIANG_CHESS_INDEX){
            GetCountryChess(pChess->GetCountry(), dstCountry);
        }
#endif
