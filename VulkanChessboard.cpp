...
void VulkanChessboard::UpdateFontUniform(VkDevice device, uint32_t playerCountry){
    glm::vec2 center;
    if(playerCountry == WU_COUNTRY_INDEX){
        ...
        center = glm::vec2(1, CHESSBOARD_ROW / 2);
    }
    std::array<glm::vec2, 3>pos = { glm::vec2(1000, 1000), glm::vec2(1000, 1000), glm::vec2(1000, 1000) };
    if(playerCountry != -1){
        //吴在九宫格的中心点
        pos[WU_COUNTRY_INDEX] = glm::vec2(COLUMN_TO_X(center.x), ROW_TO_Y(center.y));
        if(center.x == 1){
            //汉在汉的位置上
            pos[WEI_COUNTRY_INDEX] = glm::vec2(COLUMN_TO_X(center.x - 1), ROW_TO_Y(center.y + 1));
            pos[SHU_COUNTRY_INDEX] = glm::vec2(COLUMN_TO_X(center.x - 1), ROW_TO_Y(center.y - 1));
        }
        else if(center.y == 1){
            //汉在魏的位置上
            pos[WEI_COUNTRY_INDEX] = glm::vec2(COLUMN_TO_X(center.x - 1), ROW_TO_Y(center.y - 1));
...
void VulkanChessboard::CreateFontResource(VulkanDevice device, VkQueue graphics, VulkanPool pool){
    ...
    for (size_t i = 0; i < fontImageCount; ++i){
        ...
        font::GetFontImageData(fontBuffer, ALLIANCE_POINT_FONT_WIDTH, ALLIANCE_POINT_FONT_HEIGHT, word[i], datas[i]);
    }
...
void VulkanChessboard::SetupDescriptorSet(VkDevice device, VkDescriptorSetLayout setLayout, VulkanPool pool){
    ...
    pool.AllocateDescriptorSets(device, &setLayout, 1, &descriptorSet.alliance);
    pool.AllocateDescriptorSets(device, &setLayout, 1, &descriptorSet.background);
    pool.AllocateDescriptorSets(device, &setLayout, 1, &fonts.descriptrorSet.alliance);
...
void VulkanChessboard::DrawAllianceFont(VkCommandBuffer command, const VkPipelineLayout &layout){
    uint32_t dynamicOffsets;
